...
public class RestAPIVerticle extends AbstractVerticle {
	...
	public void start(Promise<Void> startFuture) {
		...
		MqttClientOptions options = new MqttClientOptions().setAutoKeepAlive(true);
		mqttClient = MqttClient.create(vertx, options);
		mqttClient.connect(1883, "localhost", s -> {
			if (s.succeeded()) {
				System.out.println("✅ Conectado al broker MQTT");
			} else {
				System.out.println("❌ Error al conectar al broker MQTT: " + s.cause().getMessage());
			}
		});
		...
		router.route().handler(ctx -> {
		    System.out.println("🛑 LLEGA PETICIÓN: " + ctx.request().method() + " " + ctx.request().path());
		    ctx.next();
		});
		...
		System.out.println("✅ Ruta POST /api/groups registrada");
	...
	private void addSensorValue(RoutingContext routingContext) {
		...
		vertx.eventBus().request(RestEntityMessage.SensorValue.getAddress(), gson.toJson(databaseMessage), handler -> {
			if (handler.succeeded()) {
				...
				//MQTT
				String jsonValue = gson.toJson(sensorValue);
				String topic = "sensor/" + sensorValue.getIdSensor() + "/valor";
				mqttClient.publish(
					topic,
					io.vertx.core.buffer.Buffer.buffer(jsonValue),
					io.netty.handler.codec.mqtt.MqttQoS.AT_LEAST_ONCE,
					false,
					false,
					pubResult -> {
						if (pubResult.succeeded()) {
							System.out.println("Valor del sensor publicado por MQTT en: " + topic);
						} else {
							System.out.println("Error al publicar en MQTT: " + pubResult.cause().getMessage());
						}
					});
		...
		vertx.eventBus().request(RestEntityMessage.ActuatorStatus.getAddress(), gson.toJson(databaseMessage),
				handler -> {
					if (handler.succeeded()) {
						...
						Actuator actuatorCreated = gson.fromJson(responseMessage.getResponseBody(), Actuator.class);
						String jsonStatus = gson.toJson(actuatorCreated);
						String topic = "actuador/" + actuatorCreated.getIdActuator() + "/estado";
						mqttClient.publish(
						  topic,
						  io.vertx.core.buffer.Buffer.buffer(jsonStatus),
						  io.netty.handler.codec.mqtt.MqttQoS.AT_LEAST_ONCE,
						  false,
						  false,
						  pubResult -> {
						    if (pubResult.succeeded()) {
						      System.out.println("Estado del actuador publicado por MQTT en: " + topic);
						    } else {
						      System.out.println("Error al publicar en MQTT: " + pubResult.cause().getMessage());
						    }
						  });
